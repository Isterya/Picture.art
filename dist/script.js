...
const calc = (size, material, options, promocode, result) => {
  ...
  const calcFunc = () => {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      ...
      resultBlock.textContent = `Вы использовали промокод: картина будет стоить ${Math.round(sum * 0.7)} рублей`;
    } else {
...
const drop = () => {
  ...
  function highlight(item) {
    item.closest('.file_upload').style.border = '5px solid black';
    ...
    item.addEventListener('submit', e => {
      ...
      (0,_services_requests__WEBPACK_IMPORTED_MODULE_0__.postData)(api, formData).then(res => {
        ...
        setTimeout(() => {
          ...
          firstItem.classList.remove('fadeOutUp');
          firstItem.classList.add('fadeInUp');
          windows.forEach(win => {
...
// В том случае, если используется получение карточек с сервера, мы вместо аргумента "styles" получаем "wrapper".
// В данном проекте wrapper-ом является селектор "#styles .row".
...
const showMoreStyles = (trigger, styles) => {
  ...
  //       errorMessage.classList.add('error-message', 'animated', 'fadeInUp');
  //       errorMessage.style.cssText = 'color: red; text-align: center; margin-top: 20px;';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_sliders__WEBPACK_IMPORTED_MODULE_1__["default"])('.feedback-slider-item', 'horizonal', '.main-prev-btn', '.main-next-btn');
